...
class PipelineState(TypedDict, total=False):
    ...
    risk_level: str
    diff: str
    secrets_findings: list[dict[str, str | int]]
    hours: int
    widened: bool
    events_empty: bool
    final_response: str
...
def _mode_note() -> str:
    ...
    return "The system is running in LIVE mode: GitHub comments and notifications are real."
...
def build_graph(llm: Any | None = None):
    ...
    def _ask(prompt: str) -> str:
        response = llm.invoke([SystemMessage(content=_mode_note()), HumanMessage(content=prompt)])
        return str(response.content)

    def _owner_repo(payload: dict[str, Any]) -> tuple[str, str]:
    ...
    def log_input(state: PipelineState) -> dict:
        ...
        if input_type == "pull_request":
            ...
            event_id = db.insert_event(
                event_type="chat_query", action="chat", summary=state.get("text")
            )
        elif input_type == "daily_report":
            event_id = db.insert_event(
                event_type="daily_report",
                action="scheduled",
                summary="Automated daily report trigger",
            )
        else:
    ...
    def classify_risk(state: PipelineState) -> dict:
        ...
        result = json.loads(
            classify_pr_risk.invoke(
                {"pr_number": pr_number, "owner": state["owner"], "repo": state["repo"]}
            )
        )
    ...
    def deep_review(state: PipelineState) -> dict:
        diff = fetch_pr_diff.invoke(
            {"pr_number": state["pr_number"], "owner": state["owner"], "repo": state["repo"]}
        )
        body = _ask(
    ...
    def post_review_comment(state: PipelineState) -> dict:
        ...
        db.update_event_agent_response(
            state["event_id"], state["final_response"], state.get("risk_level")
        )
        return {}
    ...
    def do_fetch_push_diff(state: PipelineState) -> dict:
        ...
        if not isinstance(before, str) or not isinstance(after, str):
            raise ValueError("push payload must include before and after commit SHAs")
    ...
    def remediate(state: PipelineState) -> dict:
        branch = (state["payload"].get("ref") or "").removeprefix("refs/heads/")
        findings = state["secrets_findings"]
        findings_desc = "; ".join(f"{f['type']} ({f['redacted']})" for f in findings)

        message = (
            f"Secret-like value detected in push to "
            f"{state['owner']}/{state['repo']}@{branch}: {findings_desc}. Review required."
        )
    ...
    graph.add_conditional_edges(
        "classify_risk", route_risk, {"low": "low_risk_summary", "deep": "deep_review"}
    )
    ...
    graph.add_conditional_edges(
        "scan_secrets", route_secrets, {"found": "remediate", "clean": "clean_note"}
    )
    ...
    graph.add_conditional_edges(
        "gather_facts",
        route_after_gather,
        {"widen": "widen_window", "summarize": "summarize_report"},
    )
...
_pipeline = None
_tally_client: TallyClient | None = None
...
def get_tally_client() -> TallyClient:
    global _tally_client
    with _state_lock:
        if _tally_client is None:
            _tally_client = TallyClient.from_env()
    return _tally_client
...
def run(
    ...
    return get_agent().invoke(
        ...
        config={"callbacks": [get_tally_client().callback(source=f"github-monitor:{input_type}")]},
    )


def close() -> None:
    """Flush and close the long-lived Tally client during process shutdown."""
