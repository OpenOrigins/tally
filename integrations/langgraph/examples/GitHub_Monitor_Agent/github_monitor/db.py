...
def update_event_agent_response(
    event_id: int, agent_response: str, risk_level: str | None = None
) -> None:
    with get_conn() as conn:
...
def update_event_ai_summary(event_id: int, ai_summary: str) -> None:
...
def query_events(
    since_hours: int = 24, event_type: str | None = None, limit: int = 200
) -> list[dict]:
    ...
    with get_conn() as conn:
        if since_hours and since_hours > 0:
            ...
            if event_type:
                rows = conn.execute(
                    "SELECT * FROM events WHERE ts >= ? AND event_type = ? "
