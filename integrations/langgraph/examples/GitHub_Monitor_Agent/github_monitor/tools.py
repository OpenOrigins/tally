...
def scan_diff_for_secrets(text: str) -> str:
    """Scan added lines in a unified diff for leaked secrets -- API keys, AWS keys,
    GitHub tokens, private keys, and generic secret assignments -- using local regex
    only (no LLM call). Findings contain redacted values safe to include in alerts."""
    findings = secrets_scanner.scan_diff_for_secrets(text)
    if not findings:
