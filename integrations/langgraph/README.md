...
install `tally-langgraph` without the extra. The package directly depends on
`langchain-core`, `certifi`, and `python-dotenv`.
...
```bash
tally-langgraph connect
```

The interactive prompt hides the key while you type it. For unattended setup, provide
the key through `TALLY_API_KEY`; `--api-key` is also available but may expose the key in
...
```bash
TALLY_API_URL='https://your-tally-host/v1/tally/logs' tally-langgraph connect
```
...
`TallyClient.from_env()` reads `.env` automatically, then overlays process environment
variables. Set `TALLY_ENV_FILE` or pass `env_file=` to read another file. The library
never creates or modifies that file on its own -- only the explicit `connect` command
does so.
...
Values in the process environment override values read from `.env`.

| Environment variable | Default | Purpose |
|---|---:|---|
| `TALLY_ENV_FILE` | `.env` | Environment file read by `from_env()` and updated by the CLI |
