...
/// Sums per-message `usage` blocks out of a Claude Code transcript (JSONL) to
/// report cumulative token usage for the session recorded in that file.
/// Assistant messages are deduplicated by `message.id` since Claude Code can
/// rewrite the same message as a turn streams in.
pub fn transcript_token_usage(transcript_path: &Path) -> Value {
    ...
    let by_model: serde_json::Map<String, Value> = per_model
        .into_iter()
        .map(|(model, model_totals)| (model, model_totals.to_json()))
        .collect();
...
mod tests {
    ...
    #[test]
    fn sums_transcript_usage_and_dedupes_repeated_messages() {
        let path = env::temp_dir().join(format!("tally-transcript-{}.jsonl", unique_suffix()));
        let mut file = fs::File::create(&path).unwrap();
        // Same message id written twice, as Claude Code does while a turn streams in.
        writeln!(
            file,
            "{}",
            json!({"type": "assistant", "message": {"id": "msg_1", "model": "claude-sonnet-5", "usage": {"input_tokens": 10, "output_tokens": 2}}})
        )
        .unwrap();
        writeln!(
            file,
            "{}",
            json!({"type": "assistant", "message": {"id": "msg_1", "model": "claude-sonnet-5", "usage": {"input_tokens": 10, "output_tokens": 5}}})
        )
        .unwrap();
        writeln!(
            file,
            "{}",
            json!({"type": "assistant", "message": {"id": "msg_2", "model": "claude-haiku-4-5", "usage": {"input_tokens": 3, "output_tokens": 1, "cache_read_input_tokens": 100}}})
        )
        .unwrap();
        writeln!(file, "{}", json!({"type": "user", "message": {"content": "hi"}})).unwrap();

        let usage = transcript_token_usage(&path);
        assert_eq!(usage["available"], true);
        // The repeated msg_1 is deduped, keeping only its first-seen usage (input 10, output 2).
        assert_eq!(usage["assistant_messages"], 2);
        assert_eq!(usage["input_tokens"], 13);
        assert_eq!(usage["output_tokens"], 3);
        assert_eq!(usage["total_tokens"], 116);
        assert_eq!(usage["by_model"]["claude-sonnet-5"]["output_tokens"], 2);
        assert_eq!(usage["by_model"]["claude-haiku-4-5"]["cache_read_input_tokens"], 100);
    ...
    fn missing_transcript_reports_unavailable() {
        let usage = transcript_token_usage(&env::temp_dir().join("does-not-exist.jsonl"));
